...
function Comp1() {
  ...
  const handleIntersection = (entries: IntersectionObserverEntry[]) => {
    ...
    if (entry.isIntersecting && !hasScrolled) {
      setIsInView(true);
      setHasScrolled(true); // Ensures animations trigger only on the first scroll
    }
  };
  ...
  useEffect(() => {
    if (isInView) {
      // Smooth Feature Item Animations
      gsap.fromTo(
        ".feature-item",
      ...
      <div className="container mx-auto px-4 py-16 md:py-24 flex flex-col lg:flex-row items-center gap-12">
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="space-y-6" ref={featureRef}>
            <FeatureItem
              icon={<Sparkles />}
              text="Generate project using text or voice prompts"
            />
            <FeatureItem
              icon={<Sparkles />}
              text="Chat with project (Discuss anything)"
            />
